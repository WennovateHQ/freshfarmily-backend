 ...
 * Pricing Models
 * 
 * Defines models for the FreshFarmily pricing system including:
...
// PricingConfiguration model to store platform-wide pricing settings
const PricingConfiguration = sequelize.define('PricingConfiguration', {
  id: {
    type: DataTypes.UUID,
...
// DriverCompensationConfig model to store driver pay structure
const DriverCompensationConfig = sequelize.define('DriverCompensationConfig', {
  id: {
    type: DataTypes.UUID,
    defaultValue: DataTypes.UUIDV4,
    primaryKey: true
...
// OrderCharge model to store detailed pricing breakdown for each order
...
// DriverEarnings model to track driver compensation
...
// Membership model to track customer membership status
...
// Function to establish associations
function establishAssociations() {
  const { Order } = require('./order');
  const { User } = require('./user');
  
  // Order association
  Order.hasOne(OrderCharge, { foreignKey: 'orderId' });
  OrderCharge.belongsTo(Order, { foreignKey: 'orderId' });
  
  // Pricing configuration association
  PricingConfiguration.hasMany(OrderCharge, { foreignKey: 'pricingConfigId' });
  OrderCharge.belongsTo(PricingConfiguration, { foreignKey: 'pricingConfigId' });
  
  // Driver earnings associations
  User.hasMany(DriverEarnings, { foreignKey: 'driverId', as: 'Earnings' });
  DriverEarnings.belongsTo(User, { foreignKey: 'driverId', as: 'Driver' });
